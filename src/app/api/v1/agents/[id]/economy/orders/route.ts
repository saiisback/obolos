...
export async function POST(req:NextRequest,context:{params:Promise<{id:string}>}){try{
 const id=z.uuid().parse((await context.params).id);await authenticateAgentKey(req.headers.get('authorization'),id);await rateLimit(`economy-agent-delivery:${id}`,120,60);await rateLimit('economy-agent-delivery:global',1000,60);
...
/** Read-only identity preflight lets a local payer verify the platform owner before spending. */
export async function GET(req:NextRequest,context:{params:Promise<{id:string}>}){try{
 const id=z.uuid().parse((await context.params).id);await authenticateAgentKey(req.headers.get('authorization'),id);await rateLimit(`economy-agent-identity:${id}`,120,60);
 const users=await sql()`SELECT u.id,u.address FROM platform_agents a JOIN platform_users u ON u.id=a.user_id WHERE a.id=${id}`;
 if(!users[0])throw new PlatformError(404,'AGENT_NOT_FOUND','Agent not found.');
 return platformJson({platformAgentId:id,ownerAddress:String(users[0].address).toLowerCase()});
 }catch(error){return platformError(error);}}
