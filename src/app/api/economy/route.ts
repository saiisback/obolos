...
export async function POST(req:NextRequest){let phase='authentication';try{
 ...
 }catch(error){if(!(error instanceof PlatformError)){const candidate=error as {name?:unknown;code?:unknown};const safe=(v:unknown)=>typeof v==='string'&&/^[A-Za-z0-9_]{1,48}$/.test(v)?v:'unknown';console.error('[economy-refresh]',{phase,name:safe(candidate?.name),code:safe(candidate?.code)});}return platformError(error);}}
