...
export function calculateEconomyMetrics(events:EconomicSettlement[],basket:PriceComponent[],window:MetricWindow):EconomyMetrics {
  ...
  if(valued.some(e=>e.verifiedFinalOutputAtomic!<0n||e.verifiedIntermediateInputAtomic!<0n))throw Error('Invalid output valuation');
  const gross=selected.reduce((v,e)=>v+e.principalAtomic,0n),cost=eligible.reduce((v,e)=>v+e.principalAtomic,0n);
  ...
  if(!complete)limitations.push('GAP, surplus and productivity need explicit final-output and intermediate-input valuations for every eligible order in this window.');
  ...
  return {methodology,asset:window.asset,start:window.start,end:window.end,basketIdentity:identity,arpiBps,
    ...
    surplusAtomic:complete?revenue-cost:null,productivityBps:complete&&cost>0n?revenue*bps/cost:null,
