...
export function mandateMessage(m:MandateFields) {
  return ['Obolos isolated runner spending mandate v1',`Origin: ${m.origin}`,`Mandate: ${m.id}`,`Agent: ${m.agentId}`,`Owner: ${m.owner.toLowerCase()}`,
...
export async function validateSignedMandate(value:unknown,pin:{owner:string;agentId:string;origin:string;now?:Date}):Promise<boolean> {
  const parsed=mandateFieldsSchema.extend({message:z.string().max(5000),signature:z.string().regex(/^0x[0-9a-fA-F]{130}$/)}).strict().safeParse(value);
