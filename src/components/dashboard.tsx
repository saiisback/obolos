...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const opener = document.activeElement instanceof HTMLElement && document.activeElement !== document.body
      ...
      : document.querySelector<HTMLElement>(`[data-receipt-id="${CSS.escape(selectedReceipt.id)}"]`);
  ...
  function navigate(next: View) { setView(next); setConfigure(false); setMobileNav(false); setSelectedReceipt(null); }
  function newRun() { setConfigure(true); setView('overview'); setMobileNav(false); setError(''); }
  ...
  async function exportRun() {
    ...
    try {
      ...
      saveText(`agentgdp-${shortId(run.id)}.json`, await response.text(), 'application/json'); setNotice('Evidence bundle downloaded.');
  ...
  return <div className="app-shell">
    ...
    <aside ref={navigationRef} className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`} role={mobileNav ? 'dialog' : undefined} aria-modal={mobileNav ? true : undefined} aria-label="Workspace navigation"><button className="icon-button navigation-close" aria-label="Close navigation" onClick={() => setMobileNav(false)}><X size={18} /></button>
      <a className="brand" href="/" aria-label="AgentGDP home"><span className="brand-mark"><span /><span /><span /></span><span>AgentGDP<span className="brand-period">.</span></span></a>
      <div className="workspace-switch"><span className="workspace-avatar">A</span><span>My workspace<small>Personal workspace</small></span><LockKeyhole size={13} /></div>
      <button className="new-run" onClick={newRun}><Plus size={16} />New research run<span>↗</span></button>
      <p className="nav-label">WORKSPACE</p>
      <nav aria-label="Main navigation">
        {([{ id: 'overview', label: 'Overview', icon: LayoutDashboard }, { id: 'runs', label: 'Research runs', icon: Waypoints }, { id: 'receipts', label: 'Payments & receipts', icon: ReceiptText }, { id: 'providers', label: 'Provider directory', icon: Globe2 }] as const).map(item => <button key={item.id} className={`nav-item ${view === item.id || (item.id === 'overview' && view === 'report') ? 'active' : ''}`} onClick={() => navigate(item.id)}><item.icon size={17} strokeWidth={1.7} />{item.label}{item.id === 'runs' && !!state?.runs.length && <span className="nav-count">{state.runs.length}</span>}</button>)}
      </nav>
      <div className="sidebar-runs"><p className="nav-label">RECENT RUNS</p>{!state?.runs.length ? <p className="sidebar-empty">Your research will live here.</p> : state.runs.slice(0, 4).map(item => <button key={item.id} className={`recent-run ${selectedId === item.id && !configure ? 'selected' : ''}`} onClick={() => { setSelectedId(item.id); navigate('overview'); }}><span className={`tiny-dot ${item.status}`} /><span>{item.repos[0]?.split('/')[1] ?? 'Research run'}{item.repos.length > 1 ? ` +${item.repos.length - 1}` : ''}</span><ChevronRight size={12} /></button>)}</div>
      <div className="sidebar-bottom"><div className="workspace-note"><ShieldCheck size={19} strokeWidth={1.6} /><strong>Autonomy, with boundaries.</strong><p>You set the limits.<br />Your agents do the legwork.</p></div><button className={`nav-item ${view === 'connections' ? 'active' : ''}`} onClick={() => navigate('connections')}><Settings2 size={17} />Connections<span className="connection-dot" /></button><div className="profile"><span className="profile-avatar">YO</span><div>Your workspace<small>Local operator</small></div><span className="profile-status"><span />{state ? 'Ready' : 'Loading'}</span></div></div>
    </aside>
    <div className="app-main" inert={mobileNav ? true : undefined}>
      <header className="topbar"><div className="breadcrumb"><button className="mobile-menu icon-button" aria-label="Open navigation" onClick={() => setMobileNav(true)}><Menu size={19} /></button><span>Workspace</span><ChevronRight size={13} /><strong>{({ overview: 'Overview', runs: 'Research runs', receipts: 'Payments & receipts', providers: 'Provider directory', connections: 'Connections', report: 'Research report' })[view]}</strong></div><div className="topbar-right"><span className={`environment ${run && !configure ? run.mode : mode}`}><FlaskConical size={13} />{(run && !configure ? run.mode : mode) === 'rehearsal' ? 'Rehearsal environment' : 'Live · testnets'}</span><span className="topbar-separator" /><button className="icon-button refresh-button" aria-label="Refresh workspace" onClick={() => { void refresh(); }}><RefreshCw size={15} /></button><span className="operator-avatar">YO</span></div></header>
      ...
      {!state ? <div className="loading-state"><Loader2 className="spin" size={24} /><p>Opening your workspace…</p>{error && <button className="button" onClick={() => void refresh()}>Try again</button>}</div> : <div className={`workspace-body ${view === 'overview' || view === 'report' || (view === 'receipts' && selectedReceipt) ? 'has-inspector' : ''}`}>
        <main className="content-area">
          {view === 'overview' && <>
            {isSetup ? <>
              <div className="welcome-top"><div className="eyebrow"><span />A WORKSPACE FOR AGENT ECONOMIES</div><span className="quiet-caption">Human direction. Agent execution.</span></div>
              <section className="welcome"><div><h1>Put your agents<br />to work<span className="heading-dot">.</span></h1><p>From a research question to a verified report.<br className="desktop-break" /> Every decision bounded. Every payment accounted for.</p><div className="welcome-meta"><span><ShieldCheck size={14} />Human-defined limits</span><span><ReceiptText size={14} />Traceable spending</span></div></div><div className="workflow-art"><Image src="/illustrations/agent-workforce.png" alt="Three illustrated robot coworkers passing a payment token and a research report" width={420} height={280} priority /><span className="art-caption">SMALL TEAM. BIG FOLLOW-THROUGH.</span></div></section>
              <section className="setup-card"><div className="section-heading"><div><h2>Start a research run</h2><p>Compare public GitHub repositories with source-backed evidence.</p></div><span className="step-label">01 / SET UP</span></div>
              ...
              <section className="how-it-works"><div className="section-heading"><h2>A small team. A complete workflow.</h2><span className="quiet-caption">From mandate to verified output</span></div><div className="workflow-strip">{stages.map((item, index) => <div key={item.id}><span className="workflow-number">0{index + 1}</span><item.icon size={20} strokeWidth={1.5} /><strong>{item.name}</strong><p>{item.description}</p></div>)}</div></section>
            </> : <>
