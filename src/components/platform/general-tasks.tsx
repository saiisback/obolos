...
function TaskOutput({output}: {output: unknown}) {
  const text = output !== null && typeof output === 'object' && !Array.isArray(output) && 'text' in output && typeof output.text === 'string' ? output.text : null;
  return text === null ? <pre>{JSON.stringify(output, null, 2)}</pre> : <><pre>{text}</pre><details><summary>Service response</summary><pre>{JSON.stringify(output, null, 2)}</pre></details></>;
}
...
export function GeneralTasks({agents, selectedAgentId}: {agents: Agent[]; selectedAgentId?: string | null}) {
  ...
  return <section className={t.tasks} aria-label="General tasks">
    ...
    {loading ? <p role="status" className={t.hint}>Loading tasks…</p> : !tasks.length ? <p className={t.empty}>Your requests, approved plans, and paid results will appear here.</p> : tasks.map(task => <article key={task.id} className={t.task} aria-label={task.instruction}>
      ...
      {task.status === 'completed' && <p className={t.hint}>All approved service calls finished. Review the outputs against your request before using them.</p>}
