...
import { GeneralTasks } from './general-tasks';
...
export function Workspace() {
  ...
  return <main id="main" className={`${s.main} ${w.agentsPage}`}>
    <header className={w.pageHeading}><div><h1>{selectedAgent?.name || 'Your agents'}</h1><p>{selectedAgent ? selectedAgent.description || 'Review resource purchases and spending authorization.' : 'Give your agents a task, review a plan, and collect the result.'}</p></div>{!selectedAgent&&<WorkspaceGlyph kind="agents"/>}{selectedAgent ? <button className={s.secondary} onClick={() => setSelected(null)}>Back to agents</button> : <button ref={createRef} className={s.primary} onClick={() => setCreating(true)} disabled={creating || loading}>Create agent</button>}</header>
    {loading ? <div className={s.empty} role="status">Loading your workspace…</div> : !configured ? <div className={s.empty}><h2>Account service needs setup</h2><p>This deployment needs a database and sign-in configuration before your workspace is available.</p><button className={s.secondary} onClick={() => void load()}>Check again</button></div> : <>
      ...
      {creating && <form onSubmit={createAgent} className={s.agentForm}><div className={s.sectionHeading}><h3>New agent</h3><button type="button" className={s.textButton} disabled={saving} onClick={() => { setCreating(false); setError(''); createRef.current?.focus(); }}>Cancel</button></div><div className={s.formGrid}><label>Agent name<input ref={nameRef} name="name" required maxLength={80} placeholder="My assistant" disabled={saving} /></label><label className={s.fullWidth}>Description<textarea name="description" maxLength={1000} rows={2} placeholder="What kinds of work will this agent do?" disabled={saving} /></label><label>Data budget · HBAR per run<input name="dataBudget" required inputMode="decimal" defaultValue="0.1" aria-describedby="data-budget-hint" disabled={saving} /><small id="data-budget-hint">0–1 HBAR · Up to 8 decimal places</small></label><label>Verification budget · USDC per run<input name="verificationBudget" required inputMode="decimal" defaultValue="0.1" aria-describedby="verification-budget-hint" disabled={saving} /><small id="verification-budget-hint">0–1 test USDC · Up to 6 decimal places</small></label></div><div className={s.formFooter}><p>These are configuration limits. No funds will move.</p><button className={s.primary} disabled={saving} type="submit">{saving ? 'Creating agent…' : 'Create agent ↗'}</button></div></form>}
      {agents.length === 0 ? <div className={s.empty}><h3>Your first agent starts here.</h3><p>Give it a name, set its separate spending limits, then open its setup to connect a runner and approve spending.</p>{!creating && <button className={s.secondary} onClick={() => setCreating(true)}>Create your first agent</button>}</div> : <div className={`${s.agentList} ${!selectedAgent ? w.collectionGrid : ""}`}>{(selectedAgent ? [selectedAgent] : agents).map(agent => <article key={agent.id} className={s.agent}><div className={`${s.agentOverview} ${selectedAgent ? w.selectedOverview : ""}`}><div className={s.agentName}><WorkspaceGlyph kind="agents" compact/><h3>{agent.name}</h3><p>{agent.description || 'No description provided.'}</p><code>{agent.id}</code></div><div className={s.agentBudget}><span>Research data / run</span><strong>{fromAtomic(agent.dataBudgetAtomic, 8)} HBAR</strong></div><div className={s.agentBudget}><span>Research verification / run</span><strong>{fromAtomic(agent.verificationBudgetAtomic, 6)} USDC</strong></div><div className={s.agentAction}><span className={s.tag}>Testnet agent</span><button className={s.secondary} aria-expanded={selected === agent.id} aria-controls={`agent-setup-${agent.id}`} onClick={() => setSelected(selected === agent.id ? null : agent.id)}>{selected === agent.id ? 'Close agent' : 'Manage agent'}</button></div></div>{selected === agent.id && <div id={`agent-setup-${agent.id}`}><AgentEconomy key={`economy-${agent.id}`} agentId={agent.id} /><details className={w.setupGuide} open={!!preferredServiceId}><summary>Repository research setup and runs</summary><p>These HBAR and verification budgets apply to the repository research runner. Economy resource purchases use their separate on-chain policy.</p><AgentExecution key={agent.id} agent={agent} owner={user.address} preferredServiceId={preferredServiceId} /></details></div>}</article>)}</div>}</section></>}
    </>}
  </main>;
}
