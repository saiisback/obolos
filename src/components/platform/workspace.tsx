...
export function Workspace() {
  ...
  return <main id="main" className={`${s.main} ${w.agentsPage}`}>
    <header className={w.pageHeading}><div><h1>{selectedAgent?.name || 'Your agents'}</h1><p>{selectedAgent ? selectedAgent.description || 'Review resource purchases and spending authorization.' : 'Manage agents, review paid resource orders, and configure repository research.'}</p></div>{!selectedAgent&&<WorkspaceGlyph kind="agents"/>}{selectedAgent ? <button className={s.secondary} onClick={() => setSelected(null)}>Back to agents</button> : <button ref={createRef} className={s.primary} onClick={() => setCreating(true)} disabled={creating || loading}>Create agent</button>}</header>
