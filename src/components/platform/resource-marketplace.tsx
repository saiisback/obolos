...
export function ResourceMarketplace() {
  ...
  return <section aria-label="Resource services" className={m.resources} aria-busy={loading}>
    ...
    <div className={m.resourceIntro}><p>Resource purchases use an owner-authorized, funded private executor. Circle execution runs locally; this catalog does not initiate payments.</p><Link href="/app/developers#selling">Resource execution guide</Link><label>Resource category<select value={category} onChange={event => setCategory(event.target.value)}><option value="all">All categories</option>{Object.entries(categoryNames).map(([key, name]) => <option key={key} value={key}>{name}</option>)}</select></label></div>
    {error ? <p role="alert" className={s.error}>{error}</p> : loading ? <p role="status">Loading published resources…</p> : !visible.length ? <p className={s.empty}>No published resource services in this category.</p> : <div className={m.grid}>{visible.map(service => {
      ...
      return <article className={m.card} key={service.serviceHash}>
        <div className={m.serviceInfo}><div className={m.serviceType}><span>Resource service</span><span>{status}</span></div><h3>{categoryNames[service.category]} · {service.unit}</h3><dl><div><dt>Seller</dt><dd><a href={`https://testnet.arcscan.app/address/${service.seller}`} title={service.seller}>{service.seller.slice(0, 8)}…{service.seller.slice(-6)}</a></dd></div><div><dt>Quantity</dt><dd>{service.quantity} {service.unit}</dd></div><div><dt>Unit price</dt><dd>{usdc(service.unitPriceAtomic)} test USDC</dd></div></dl><p className={m.endpointLabel}><span>Provider endpoint</span><code>{service.endpoint}</code></p>
        ...
        </div><div className={m.purchase}><div className={m.price}><strong>{usdc(BigInt(service.quantity) * BigInt(service.unitPriceAtomic))}</strong><span>test USDC</span></div><p>Per order · {service.quantity} {service.unit}</p><small>{status === 'Provider online' ? 'Recent provider heartbeat. Delivery still requires a paid, accepted request.' : status === 'Provider unavailable' ? 'Provider heartbeat is missing or stale. Restore the provider before new work.' : 'Availability has not been verified for this endpoint.'}</small></div>
      </article>;
    })}</div>}
  </section>;
}
