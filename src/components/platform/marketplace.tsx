...
export function Marketplace() {
  ...
  return <main id="main" className={s.main}>
    <header className={s.marketHero}><div><p className={s.eyebrow}>Verification marketplace · Arc testnet</p><h1>Buy proof.<br />Earn by verifying.</h1><p>Choose a real published repository-metric verifier for your agent, or publish one tied to your signed-in wallet.</p></div><div className={s.marketStatement}><span>Hosted execution</span><strong>01</strong><p>Obolos runs one deterministic verifier for every listing. Sellers set its name, description, payout wallet, and price. Listings do not run seller code or demonstrate independent business adoption.</p></div></header>
    ...
    <section className={s.sellerSection} aria-labelledby="seller-heading"><div className={s.sectionHeading}><div><p className={s.eyebrow}>Seller desk</p><h2 id="seller-heading">Publish and manage</h2><p>Your authenticated EVM wallet is the immutable payout recipient.</p></div>{user ? <span className={s.tag} title={user.address}>{shortAddress(user.address)}</span> : <Link href="/login" className={s.primary}>Sign in to sell ↗</Link>}</div>
      {!user ? <div className={s.notice}><strong>Wallet sign-in required</strong><p>Browsing is public. Publishing, editing paused listings, and viewing earnings are scoped to the signed-in seller wallet.</p></div> : <>
        ...
        <div className={s.sellerColumns}><section><h3>Your listings <span className={s.count}>{mine.length}</span></h3>{mine.length === 0 ? <p className={s.jobsEmpty}>You have not published a service with this wallet.</p> : mine.map(service => <OwnedService key={service.id} service={service} busy={busy === service.id} onSubmit={event => update(event, service)} />)}</section><section className={s.earnings}><div><span>Confirmed earnings</span><strong>{formatMarketPrice(totalAtomic)} <small>test USDC</small></strong><p>Arc testnet · no automatic deposits or funding</p></div><h3>Settled orders</h3>{orders.length === 0 ? <p className={s.jobsEmpty}>No confirmed orders for your listings yet.</p> : orders.map(order => <div className={s.earningRow} key={order.id}><div><strong>{formatMarketPrice(order.amountAtomic)} USDC</strong><p>{order.id}</p></div><span className={s.tag}>{order.chainConfirmed ? 'Chain confirmed' : order.status}</span></div>)}</section></div>
      </>}
    </section>
  </main>;
}
