...
export function Marketplace({workspace = false}: {workspace?: boolean}) {
  ...
  return <main id="main" className={`${s.main} ${m.market} ${workspace ? m.embedded : ""}`}>
    {workspace ? <header className={m.workspaceHeading}><h1>Marketplace</h1><p>Choose a verifier for your agents. Manage your listings and the test USDC paid to your wallet.</p><div className={m.actions}><a href="#seller-heading" className={s.secondary}>Your seller desk <ArrowDown size={16} /></a><Link href="/app/evidence" className={s.textLink}>Your execution evidence</Link></div></header> : <>
