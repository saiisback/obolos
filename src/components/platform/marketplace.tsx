...
import {GeneralSellerDesk} from './general-seller-desk';
...
export function Marketplace({workspace = false}: {workspace?: boolean}) {
  ...
  return <main id="main" className={`${s.main} ${m.market} ${workspace ? m.embedded : ""}`}>
    {workspace ? <header className={m.workspaceHeading}><h1>Marketplace</h1><p>Browse resource services and repository verifiers, inspect purchases, or publish your own capabilities.</p></header> : <>
    ...
    <section hidden={workspace && section !== 'seller-heading'} className={m.sellerSection} aria-labelledby="seller-heading"><div className={s.sectionHeading}><div><h2 id="seller-heading">Your seller desk</h2><p>Your authenticated EVM wallet is the immutable payout recipient.</p></div>{loading ? <span className={s.tag}>Checking session…</span> : user ? <span className={s.tag} title={user.address}>{shortAddress(user.address)}</span> : <Link href="/login" className={s.primary}>Sign in to sell</Link>}</div>
      {loading ? <p className={s.caption}>Loading your seller workspace…</p> : !user ? <div className={m.signin}><strong>Wallet sign-in required</strong><p>Browsing is public. Publishing, editing paused listings, and viewing earnings are scoped to the signed-in seller wallet.</p></div> : <>
        <GeneralSellerDesk/><h3>Repository-verifier listings</h3>{workspace && <div className={m.publishAction}><p>Manage listing terms and inspect payments received by your wallet.</p><button className={s.primary} onClick={() => setPublishing(value => !value)} disabled={!!busy}>{publishing ? 'Cancel verifier listing' : 'Publish a repository verifier'}</button></div>}
