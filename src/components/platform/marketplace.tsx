...
export function Marketplace({workspace = false}: {workspace?: boolean}) {
  const section = useWorkspaceSection(['catalog', 'seller-heading'], 'catalog');
  ...
  return <main id="main" className={`${s.main} ${m.market} ${workspace ? m.embedded : ""}`}>
    {workspace ? <header className={m.workspaceHeading}><h1>Marketplace</h1><p>Find a verifier for your agent or manage the services you sell.</p></header> : <>
    ...
    {workspace && <WorkspaceSections current={section} label="Marketplace sections" items={[{id:'catalog',label:'Browse verifiers'},{id:'seller-heading',label:'Your seller desk'}]}/>}
    ...
    <details hidden={workspace && section !== 'catalog'} className={m.funding}><summary>How funding and execution work <ChevronDown size={18} /></summary><p>Obolos runs the verifier. You choose its name, description, price, and receive payments in your signed-in wallet. Listings do not run seller-provided code.</p><div className={m.roles}><div><strong>Owner sets scope and signs</strong><p>A mandate is an allowance. It does not fund the runner.</p></div><div><strong>Runner executes and settles</strong><p>Your private runner and broker must be online and separately funded.</p></div><div><strong>Seller receives test USDC</strong><p>Confirmed hosted-verifier orders accrue to the listing wallet.</p></div></div><a href={workspace ? "/app/developers#runner" : "/developers#runner"}>Read the runner setup guide <ArrowUpRight size={14} /></a></details>
