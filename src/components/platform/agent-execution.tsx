...
type Job = { id: string; status: string; repos: string[]; createdAt: string; result?: Run | null; error?: string | null; receiptVerification?: 'runner-confirmed' | 'chain-confirmed' | null };
...
export function AgentExecution({ agent, owner }: { agent: Agent; owner: string }) {
  ...
  useEffect(() => {
    ...
    api<{ services: MarketService[] }>('/api/market/services', { signal: controller.signal }).then(result => {
      ...
      setVerificationServiceId(current => current || result.services.find(service => service.priceAtomic <= agent.verificationBudgetAtomic)?.id || '');
    ...
    return () => controller.abort();
  }, [agent.verificationBudgetAtomic]);
...
function JobResult({ job }: { job: Job }) {
  ...
  return <details className={s.jobResult} open={activeStatus(job.status)}><summary><span><strong>{job.repos.join(', ')}</strong><small>{dateLabel(job.createdAt)} · {job.id}</small></span><span className={s.tag}>{job.status.replaceAll('_', ' ')}</span></summary><div className={s.jobBody}>
    ...
    {result?.report && <section><h4>{result.report.title}</h4><p className={s.reportText}>{result.report.summary}</p><p className={s.reportText}>{result.report.recommendation}</p><p className={s.caption}>Generated by {result.report.generatedBy} · {result.mandate.verificationService ? 'Claim-level checks returned by the selected hosted verifier' : 'Runner-reported checks'}</p>{result.report.checks?.map((check, index) => <div className={s.sourceCheck} key={index}><strong>{check.passed ? 'Passed' : 'Failed'} · {check.label}</strong><p>{check.detail}</p></div>)}</section>}
