...
export function AgentEconomy({agentId, showPolicy = true}: {agentId: string; showPolicy?: boolean}) {
  return <AgentEconomyContent key={agentId} agentId={agentId} showPolicy={showPolicy} />;
}

function AgentEconomyContent({agentId, showPolicy}: {agentId: string; showPolicy: boolean}) {
  ...
  const policy = showPolicy ? activity?.policy : undefined;
  const Heading = showPolicy ? "h3" : "h2";
  const sourceNote = <p className={s.caption}>Seller and buyer receipts reflect finalized indexed evidence{activity?.indexedAt ? ` last refreshed ${new Date(activity.indexedAt).toLocaleString()}` : '; the index has not been refreshed'}. An absent receipt means it has not been observed in this index.</p>;
  return <section className={`${s.execution} ${showPolicy ? '' : e.resource}`} aria-label={showPolicy ? "Economy activity" : "Resource evidence"}>
    <div className={`${s.sectionHeading} ${showPolicy ? '' : e.resourceHeading}`}><div><Heading>{showPolicy ? "Economy activity" : "Resource evidence"}</Heading><p>{showPolicy ? "Paid resource orders and finalized spending authorization." : "Paid orders, provider output, and indexed seller and buyer receipts."}</p></div><button className={s.secondary} aria-label={showPolicy ? undefined : "Refresh resources"} disabled={loading} onClick={() => setRevision(value => value + 1)}>{showPolicy ? (loading ? 'Refreshing economy…' : 'Refresh economy') : (loading ? 'Refreshing…' : 'Refresh')}</button></div>
    {error && <p className={s.error} role="alert">{error}</p>}
    {loading && !activity && <p role="status">{showPolicy ? "Loading economy orders and policy…" : "Loading resource evidence…"}</p>}
    ...
    {activity && <>{showPolicy ? sourceNote : <details className={e.indexNote}><summary>About indexed receipts</summary>{sourceNote}</details>}
