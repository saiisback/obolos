...
export default function LiveConnections({ operatorAuthenticated }: { operatorAuthenticated: boolean }) {
  ...
  const evidence = overview ? [
    ...
    { label: 'Ledger approvals', count: overview.evidence.ledgerApprovals },
  ] : [];
  ...
  return <section className="live-connections" aria-labelledby="live-readiness-title" aria-busy={loading}>
    ...
    {overview && <>
      ...
      {canViewWallets && <div className="live-controller"><div><Fingerprint size={20} /><strong>Ledger mandate controller</strong></div><dl><PublicAddress label="Controller address" value={overview.controllerAddress} onCopy={value => void copy(value)} /></dl><p>The controller signs spending-limit escalations on the physical Ledger. Its address is distinct from the payment wallets.</p></div>}
