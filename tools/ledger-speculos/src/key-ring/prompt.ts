...
export async function promptHidden(label: string): Promise<PromptResult> {
  ...
  throw new Error(
    "Password required but no TTY available and WALLET_PASS is not set.\n" +
      ...
      "  macOS : WALLET_PASS=$(security find-generic-password -a default -s obolos-speculos-ring-password -w) wallet-cli ...\n" +
      "  Linux : WALLET_PASS=$(secret-tool lookup service obolos-speculos-ring-password account default) wallet-cli ...\n" +
      "  Win   : $env:WALLET_PASS=(Get-StoredCredential -Target obolos-speculos-ring-password).GetNetworkCredential().Password; wallet-cli ...",
  );
}
