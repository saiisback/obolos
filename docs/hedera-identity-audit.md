...
`HederaAgentIdentity` carries canonical public data, UAID, restricted submit key and optional topic/profile proof. `HcsAnchor` carries testnet network, topic ID, submission transaction ID, sequence number, consensus timestamp and the fixed mirror URL. A generated UAID alone is a local identifier. Registration is chain confirmed only after the topic's exact submit key and the message bytes, sequence, timestamp and successful transaction binding all pass mirror verification.
