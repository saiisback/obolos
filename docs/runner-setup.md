...
4. Follow [Circle login and funding](live-setup.md#6-log-in-to-the-circle-agent-wallet-yourself) under the broker account. Export the same private `CIRCLE_CLI_HOME` while logging in that the broker configuration uses; complete the email OTP yourself. Pin your returned Arc **agent** wallet as `CIRCLE_WALLET_ADDRESS` and fund it with testnet USDC. `ARC_VERIFIER_ADDRESS` and `ARC_VERIFICATION_FEE_ATOMIC=50000` remain required for legacy v1 execution. Marketplace v2 execution instead pays the exact recipient and price signed into the selected service snapshot. The browser owner EOA, Hedera payer and Circle wallet have separate roles and can have different addresses.
...
6. Pin the hosted marketplace and its authorized owner in the private `.env.broker`. The marketplace owner must be the same wallet you set as `RUNNER_OWNER_ADDRESS` below:
...
5. In the agent setup panel, select a published verification service, prepare the spending mandate, inspect its exact message, and sign with your owner wallet. The v2 message binds the platform, agent, mandate nonce, repositories, providers, per-repository price cap, separate per-run HBAR/USDC caps, maximum run count, derived total allowances, expiry, and the service's ID, revision, recipient, price, and fixed endpoint. It authorizes Hedera and Arc **testnet** execution only, for at most 24 hours and 10 runs. Queue a job only after you have checked these terms and the available testnet funds. Existing v1 signed messages remain compatible; newly prepared web mandates require a selected live service.
