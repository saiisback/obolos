...
The model sees a compact capability catalog with short `service_N_category` selectors. The private process resolves each exact selector against the same validated catalog snapshot and then applies the existing service-hash, schema, routing and budget checks. Opaque hashes and repeated payment-authority fields are not copied by the model. Unknown selectors fail closed; saved plans and payment journals still retain the complete immutable definitions and hashes.
