...
it('accepts matched live receipts and replaces supplied explorer destinations',async()=>{
 ...
 const checked=validateRunnerResult(r,{id:m.id,repos:m.repos,mandate:m});expect(checked.status).toBe('succeeded');expect(checked.result.receipts[0].explorerUrl).toBe('https://hashscan.io/testnet/transaction/0.0.123%401234567890.000000001');expect(checked.result.receipts[1].explorerUrl).toBe(`https://testnet.arcscan.app/tx/0x${'12'.repeat(32)}`);
});
