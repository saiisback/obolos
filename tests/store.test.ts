...
describe('persistent run isolation',()=>{
 it('isolates sessions and serializes competing actions',async()=>{
  const directory=await mkdtemp(join(tmpdir(),'ag-store-')),store=new RunStore(directory),run=createRun({repos:['vercel/next.js'],mode:'rehearsal'});
  ...
  await Promise.all(Array.from({length:8},()=>store.mutate('alice',run.id,r=>advanceRun(r,rehearsalGateway))));
