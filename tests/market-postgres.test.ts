...
describe.skipIf(!process.env.TEST_DATABASE_URL)('PostgreSQL marketplace',()=>{
 ...
 it('enforces seller ownership and blocks repriced signed selections',async()=>{const f=await setup();await expect(updateMarketService(f.buyer.user,f.service.id,{priceAtomic:2000})).rejects.toMatchObject({status:404});await expect(createMarketService(f.seller.user,{name:'Bad',description:'',priceAtomic:1000,recipient:f.buyer.user.address})).rejects.toThrow();const running=await f.job();await updateMarketService(f.seller.user,f.service.id,{priceAtomic:2000});await expect(f.job()).rejects.toMatchObject({code:'MANDATE_REQUIRED'});await expect(createMarketOrder(f.runner,{jobId:running.id,payer:f.buyer.user.address,dataTransactionId:await f.purchase(),report:f.report})).rejects.toMatchObject({code:'SERVICE_CHANGED'});});
