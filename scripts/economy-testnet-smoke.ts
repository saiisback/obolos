...
import {createWalletClient,defineChain,encodeFunctionData,http,keccak256,parseAbi,toHex,type Abi,type Address,type Hex} from 'viem';
...
async function main(){
 const mode=process.argv[2];assert(['prepare','purchase','acknowledge'].includes(mode),'Use prepare, purchase, or acknowledge');
 ...
 if(mode==='purchase'){
  ...
  state.request=request;await save('state.json',state);const result=await buyerApi.request('/api/economy/orders',{platformAgentId:state.platformAgentId,request});assert.equal(result.order.state,'fulfilled');state.delivery=result.order;await save('state.json',state);console.log(JSON.stringify(result));
 }
 if(mode==='acknowledge'){
